...
export default function MainSection({ regiones }: MainSectionProps) {
  const {
    register,
    setValue,
    formState: { errors },
  ...
  const { mutate: createPosta } = useMutation<PostaSchemaType>({
    mutationFn: async (data, url) => {
      ...
      const res = await axios.post(`${url}/posta`, {
        ...rest,
    ...
    onSuccess: () => {
      notify.success({ message: 'Posta creada correctamente' });
      router.push('/posta');
    },
    onError: () => {
      notify.error({ message: 'Error al crear la posta' });
    },
  });
  ...
  return (
    <form className="flex w-full flex-col gap-y-4 p-5" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Title
        icon={<LuBuilding2 />}
        title="Nueva Posta"
        description="Completa el formulario para crear una nueva posta"
      />
