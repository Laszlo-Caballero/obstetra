...
import { TipoConsultaSchema, TipoConsultaSchemaType } from '@/schemas/ayuda/tipo.schema';
import { useMutation } from '@/hooks/useMutation';
import axios from 'axios';
import { notify } from '@/libs/toast';

interface CrearTipoProps {
  onClose?: () => void;
}

export default function CrearTipo({ onClose }: CrearTipoProps) {
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const { mutate: create } = useMutation<TipoConsultaSchemaType>({
    mutationFn: async (data, urlApi) => {
      const res = await axios.post(`${urlApi}/ayuda/tipo-consulta`, data);

      return res.data;
    },
    onSuccess: () => {
      notify.success({
        message: 'Tipo de Consulta creado con éxito',
      });
      onClose?.();
    },
    onError: () => {
      notify.error({
        message: 'Error al crear el tipo de consulta',
      });
    },
  });

  const onSubmit = (data: TipoConsultaSchemaType) => {
    create(data);
  };
  return (
      ...
      <form className="flex flex-col" onSubmit={handleSubmit(onSubmit)}>
        ...
        <ModalFooter nota="Se le notificara a todos los administradores">
          <ContainerButton>
            ...
            <Button className="bg-ob-teal font-semibold" type="submit">
              <LuSave size={18} />
              Guardar
            </Button>
          </ContainerButton>
