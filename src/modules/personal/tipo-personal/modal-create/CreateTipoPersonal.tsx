...
export default function CreateTipoPersonal({ onClose }: CreateTipoPersonalProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const { mutate } = useMutation<TipoPersonalInput, Response<ResponseTipoPersonal[]>>({
    mutationFn: async (data) => {
      const res = await axios.post(`${env.url_api}/tipo-personal`, data, {
        headers: {
  ...
  return (
    ...
    >
      <ModalHeader>
        <ModalTitle title="Crear Tipo Personal" badge="Crear">
